...
with tabs[3]:
    ...
    wrk = ch3.number_input("Worker Qty", 10)
    ...
    # تم إضافة مؤشر إجمالي الرواتب هنا ليكون واضحاً
    c_p1, c_p2, c_p3 = st.columns(3)
    admin_exp = c_p1.number_input("Monthly Admin Exp", 40000)
    c_p2.metric("💰 Total Monthly Payroll", f"SAR {payroll:,.0f}")
    c_p3.metric("⚡ Annual Power Cost", f"SAR {pwr_cost:,.0f}")
...
with tabs[4]:
    ...
    std_width = ct2.number_input("Web Width (m) for Calc", 1.0)
    ...
    st.markdown("### 🧻 Raw Material Purchasing (Meters)")
    if mat_needs:
        df_mat = pd.DataFrame([{"Material": k, "Total Meters": f"{v:,.0f} m"} for k, v in mat_needs.items()])
